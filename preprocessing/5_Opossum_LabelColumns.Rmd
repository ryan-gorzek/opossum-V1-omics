...
obj.ref.glutamatergic <- ClusterWithSCT(obj.ref.glutamatergic, c(0.3))
...
```{r load_spatial}
obj.ctx <- readRDS(paste0(dir.list$spatial$seurat$processed, "opossum_combined_columns.rds"))
...
p <- DimPlot(obj.i.class.r1, group.by = "class_nn", pt.size = 0.1, 
  ...
  xlim(-12, 15) + ylim(-13, 14) + coord_equal()
print(p)
SavePNGandSVG(p, dir.list$figS4$plots, "S4E_Opossum-ClassRound1-UMAP")
```
...
p <- DimPlot(obj.i.class.r2, group.by = "class_nn", pt.size = 0.1, 
        cols = class.colors, na.value = "#505050") + 
  xlim(-15, 13) + ylim(-11, 17) + coord_equal()
print(p)
SavePNGandSVG(p, dir.list$figS4$plots, "S4F_Opossum-ClassRound2-UMAP")
```
...
print(p)
SavePNGandSVG(p, dir.list$figS4$plots, "S4G_Opossum-ClassRound3-UMAP")
```
...
p <- DimPlot(obj.i.sub.r1, group.by = "subclass_nn", pt.size = 0.5, 
  ...
  xlim(-11, 14) + ylim(-12, 12) + coord_equal()
print(p)
SavePNGandSVG(p, dir.list$figS4$plots, "S4H_Opossum-SubclassRound1-UMAP")
```
...
p <- DimPlot(obj.i.sub.r2, group.by = "subclass_nn", pt.size = 0.5, 
        cols = subclass.colors, na.value = "#8F8F8F", label = TRUE) + 
  xlim(-11, 14) + ylim(-11, 13) + coord_equal()
print(p)
SavePNGandSVG(p, dir.list$figS4$plots, "S4I_Opossum-SubclassRound2-UMAP")
```
...
## Subclass Round 3: 50% of 50 NN, No Subsample
...
print(p)
SavePNGandSVG(p, dir.list$figS4$plots, "S4J_Opossum-SubclassRound3-UMAP")
```
...
# Final Integration for Cluster Labels
...
assigned_clusters <- obj.i$SCT_snn_res.0.3_nn[!is.na(obj.i$SCT_snn_res.0.3_nn)]
cell_ids <- names(assigned_clusters)
obj.ctx$SCT_snn_res.0.3_nn <- NA
obj.ctx$SCT_snn_res.0.3_nn[cell_ids] <- assigned_clusters

cat("Cluster labels transferred to spatial object\n")
```

```{r plot_cluster_labels}
DimPlot(obj.i, group.by = "SCT_snn_res.0.2_nn", label = TRUE) + 
  NoLegend() + coord_equal()
DimPlot(obj.i, group.by = "SCT_snn_res.0.3_nn", label = TRUE) + 
  NoLegend() + coord_equal()
```
...
```{r save_integrated}
saveRDS(obj.i, paste0(dir.list$spatial$seurat$processed, "opossum_stereoseq_integrated.rds"))
cat("Saved integrated object\n")

saveRDS(obj.ctx, paste0(dir.list$spatial$seurat$processed, "opossum_stereoseq_labeled.rds"))
