...
subclasses_to_plot <- c("L2/3", "L4", "L5IT", "L5NP", "L5PT", "L6IT", "L6CT", "L6b", "OD")
...
colors[2] <- "#6d6e71"
names(colors) <- c("L2/3", "L4", "L5IT", "L5NP", "L5PT", "L6IT", "L6CT", "L6b")
colors[["OD"]] <- "#B5542A"

p <- ImageDimPlot(obj.section.plot, cols = colors, size = 3, fov = "FOV") +
  ggtitle(paste0(section.name, " - Labeled Subclasses"))
print(p)
SavePNGandSVG(p, dir.list$fig3$plots, "3A_Mouse-Section-Subclass-Spatial")
```

```{r}
p <- ImageDimPlot(obj.section.mouse, size = 3, fov = "FOV") +
  ggtitle(paste0(section.name, " - Labeled Subclasses"))
print(p)
```
...
output.file <- paste0(dir.list$spatial$seurat$processed, "mouse_", tolower(section.name), "_labeled.rds")
saveRDS(obj.section, output.file)
cat(sprintf("\nSaved labeled section to %s\n", output.file))

# Save labels separately
labels.file <- paste0(dir.list$spatial$seurat$processed, "mouse_", tolower(section.name), "_labels.csv")
